const API = 'https://vali.liara.run';

export const apiBase = API;

export function getSession(){
  try {
    return JSON.parse(localStorage.getItem('vali_session') || 'null');
  } catch {
    return null;
  }
}
...
export function setSession(session){
  localStorage.setItem('vali_session', JSON.stringify(session));
}
...
export async function canOfflineLogin(username, password){
  const saved = JSON.parse(
    localStorage.getItem('vali_offline_verifier') || 'null'
  );

  if(!saved) return false;

  return (
    saved.username === username.trim().toLowerCase() &&
    saved.verifier === await sha256(
      `${username.trim().toLowerCase()}::${password}`
    )
  );
}
...
export function queueExpense(payload){
  const q = JSON.parse(
    localStorage.getItem('vali_outbox') || '[]'
  );
